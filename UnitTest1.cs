...
*/
        ...
        public static string ProjectPath()
        {
            ...
            // Find the index of the "bin" substring
            int binIndex = current_directory.IndexOf("bin");

            // Extract the substring before "bin"
            string projectPath = current_directory.Substring(0, binIndex);
...
*/
        ...
        public void CCCLoginPage()
        {
           // string cccLoginPageReport = Path.Combine(@"C:\Users\Pearlson\source\repos\Axe\Report", "LoginReport.html");
            webDriver.Navigate().GoToUrl("https://www.w3schools.com/html/default.asp");
            Thread.Sleep(8000);

            String folderpath = ProjectPath();
            string designationPath = folderpath + "Report\\" + "DonationPage.html";

            AxeResult axeFundraiserListPageResult = new AxeBuilder(webDriver).Analyze();
            webDriver.CreateAxeHtmlReport(axeFundraiserListPageResult, designationPath, ReportTypes.Violations);
        }
      ...
      /*  [Test]
        ...
        public void DonationCOmponent()
        {
            string donationCOmponentPageReport = Path.Combine(@"C:\Users\Pearlson\source\repos\Axe\Report", "DonationComponentReport.html");
            webDriver.Navigate().GoToUrl("https://publish-p92453-e823714.adobeaemcloud.com/content/beta/en/qatestingdonationcomponent.html");
            Thread.Sleep(8000);
           // var wait = new WebDriverWait(webDriver, TimeSpan.FromSeconds(10));
         //   var particularWebElement = wait.Until(drv => webDriver.FindElement(By.XPath("//button[@class='button button--secondary button--suffix-icon']")));
            AxeResult axeFundraiserListPageResult = new AxeBuilder(webDriver).Analyze();
            webDriver.CreateAxeHtmlReport(axeFundraiserListPageResult, donationCOmponentPageReport, ReportTypes.Violations);
        }
        [Test]
        public void DonationPraticularComponent() {
            string donationCOmponentPageReport = Path.Combine(@"C:\Users\Pearlson\source\repos\Axe\Report", "TestDonationComponentReport.html");
            webDriver.Navigate().GoToUrl("https://publish-p92453-e823714.adobeaemcloud.com/content/beta/en/qatestingdonationcomponent.html");
            Thread.Sleep(8000);
            var wait = new WebDriverWait(webDriver, TimeSpan.FromSeconds(10));
            var particularWebElement = wait.Until(drv => webDriver.FindElement(By.XPath("//div[@class='donation-wrappercontainer donation-componentwrapper']")));
            webDriver.CreateAxeHtmlReport(particularWebElement, donationCOmponentPageReport, ReportTypes.Violations);
        }
